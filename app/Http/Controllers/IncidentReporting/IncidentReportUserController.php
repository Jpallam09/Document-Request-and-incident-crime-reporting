    ...
    class IncidentReportUserController extends Controller
    {
        ...
        public function requestDelete(IncidentReportUser $incidentReportUser): RedirectResponse
        {
            ...
            if ($existingRequest) {
                Alert::error('Unauthorized', 'You already sent a delete request for this report.');
                return redirect()->back();
            }
            // Create a new delete request with report snapshot data
            DeleteRequest::create([
                'user_id' => Auth::id(),
                'report_id' => $incidentReportUser->id,
                'report_title' => $incidentReportUser->title,
                'report_date' => $incidentReportUser->date,
                'report_type' => $incidentReportUser->type,
                'report_description' => $incidentReportUser->description,
                'requested_image' => null, // Optional: add logic to upload if needed
                'reason' => 'User requested to delete the report.', // Or use request input if coming from form
                'status' => 'pending',
                'requested_at' => now(),
            ]);
