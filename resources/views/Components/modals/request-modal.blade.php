  ...
  <div class="edit-request-dialog">
    <div class="edit-request-content">
      ...
      <div class="edit-request-body">
        <div class="edit-request-columns">
          ...
          <div class="edit-original-report">
            ...
            <div><strong>Date:</strong> {{ \Carbon\Carbon::parse($report->incident_date)->format('F d, Y') }}</div>
          ...
          <div class="edit-requested-report">
            ...
            <div class="edit-request-attachments">
              @foreach (json_decode($request->requested_images ?? '[]', true) as $image)
                <img src="{{ asset('storage/' . $image) }}" class="edit-thumbnail" alt="Requested Image">
              @endforeach
            </div>
          </div>
        </div>
      ...
      <div class="edit-request-footer">
            ...
            <form method="POST" action="{{ route('reporting.staff.updateRequest.accept', ['id' => $request->id]) }}" class="form-accept">
                @csrf
                <button type="submit" class="btn-accept">Accept</button>
            </form>

            <!-- Reject Button inside Modal -->
            <form method="POST" action="{{ route('reporting.staff.updateRequest.reject', ['id' => $request->id]) }}" class="form-reject">
                @csrf
